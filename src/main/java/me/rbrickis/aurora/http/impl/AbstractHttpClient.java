...
public class AbstractHttpClient implements HttpClient {
    ...
    public AbstractHttpClient(URL url, RequestType type, Map<String, String> requestProperties,
        HttpParameters parameters, byte[] body) {
        this.url = url;
        this.type = type;
    ...
    private FutureTask<HttpResponse> call() {
        Callable<HttpResponse> caller = () -> {
            ...
            return new HttpResponseBuilder().withHeaders(connection.getHeaderFields())
                .withInputStream(connection.getInputStream()).build();
        };
